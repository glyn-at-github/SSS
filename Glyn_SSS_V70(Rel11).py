...
# Allocates satellites to user areas and manages handovers between satellites
...
def availsats(i, s, df2, sat_names):

    # Lists the names of the available satellites
    # Outputs:
    # sat_list (names of each available satellite by user area)

    j = 0                                           # temporary counter along Columns (Satellites)
    sat_list = []

    while j < s:
        x = df2.iloc[i, 7+j]
        if x == 1:
            sat_list = sat_list + [sat_names[j]]
        j = j + 1
...
def allocatesat(s, n, counter, df, sat_names, h):

    # Satellite allocation algorithm
    # Takes the inputs and decides which satellite to allocate to each user area
    # Currently optimises for minimum no of handovers using the lookahead function
    # Inputs:
    # s - the number of satellites e.g. 16
    # n - the number of user areas e.g. 4
    # utc_off - the currect utc offset epoch for which the satellite needs allocating
    # df - the full dataframe for all epochs
    # sat_names - the list of satellites names in the form ["S11", "S12", "S13", "S14".......]
    # Outputs:
    # "cur_sat" and "ho_utc" columns in the df

    utc_df = df.loc[counter:counter+(n-1), :].copy()         # Create working df with just the relevant utc_off epoch
    utc_df = utc_df.reset_index(drop=True)

    # Repeat this loop incrementing i (user area) until no UA is unallocated a satellite

    while "None" in list(utc_df["cur_sat"]) or "H/O" in list(utc_df["cur_sat"]):   # Repeat until everything allocated
        i = int(0)  # UA counter

        while i < n:                                            # Loop through to allocate ERR, H/O and Sat No.

            # Takes just the sat availability columns for the UA and
            # counts how many satellites are available to the UA

            avail_sats_count = sum(utc_df.iloc[i, 7:7+s])

            # Calls the function to get satellite name(s)
            ...
            if avail_sats_count == 0:
                utc_df.at[i, "cur_sat"] = "ERR"                # Produces error message if no satellites available

            # If only 1 satellite is available, allocate it to the UA and remove it from availability

            if avail_sats_count == 1:
                sat = avail_sat_list[0]                         # Returns the available sat name in avail_sat_list df
                utc_df.at[i, "cur_sat"] = sat                   # Allocate 1st sat to UA.

                utc_df[sat] = 0                                 # Removes the allocated sat from all UAs
                utc_df.at[i, sat] = 1                           # Restores the allocated satellite to the chosen UA

                # Check whether the satellite is its own prev_sat
                # if it's not, then it must be a Handover so set the Handover utc time

                if utc_df.at[i, "cur_sat"] != utc_df.loc[i, "prev_sat"]:
                    utc_df.at[i, "ho_utc"] = utc_df.loc[i, "utc_off"] + h

            # If 2 or more satellites are available, then:
            #  If one of the available satellites is its own prev_sat then allocate that
            #  If it's not, then mark the UA for Handover and do all other UAs first
            #  If it is marked for Handover "H/O" then allocate an unallocated satellite to it using lookahead

            if avail_sats_count > 1:

                if utc_df.at[i, "cur_sat"] == "H/O":            # Is cur_sat already marked as handover?
                    avail_unalloc_sats = set(avail_sat_list)    # Find unallocated

                    # If there is more than one, need to pick the satellite which is available for the longest.
                    # Need to do this by looking down the df into subsequent utc_offsets and finding the furthest

                    longest_sat = lookahead(i, n, counter, df, avail_unalloc_sats)  # Find the longest lasting sat

                    utc_df.at[i, "cur_sat"] = longest_sat       # Populates cur_sat
                    columns = utc_df.columns

                    # Determinme the index position of longest sat
                    ...
                    # THERE NEEDS TO BE A LOCKDOWN ADDED TO ENSURE THAT IF THE NEXT SCHEDULING EPOC IS WITHIN ho_utc
                    # prev_sat DOES NOT GET ALLOCATED TO SOME OTHER UA!!

                    utc_df.at[i, "ho_utc"] = utc_df.loc[i, "utc_off"] + h  # Sets the utc_off for end of handover time

                if utc_df.at[i, "cur_sat"] == "None":

                    # Now check whether one of the available satellites is the UAs prev_sat.
                    # If so, then allocate it. If not, then it needs a handover.
                    # This approach holds on to the same satellite for as long as possible but could be changed

                    prev_sat = utc_df.at[i, "prev_sat"]

                    if prev_sat in avail_sat_list:

                        # Allocate the prev_sat to cur_sat (No Handover)

                        utc_df.at[i, "cur_sat"] = prev_sat
                        columns = utc_df.columns
                        column_index = columns.get_loc(prev_sat)

                        # Remove the allocated satellite from all UAs and
                        # Restore the allocated satellite to the chosen UA

                        utc_df[utc_df.columns[column_index]] = 0
                        utc_df.iat[i, column_index] = 1

                    else:

                        # Mark the UA as a handover and
                        # set the h/o utc time

                        utc_df.at[i, "cur_sat"] = "H/O"
                        utc_df.at[i, "ho_utc"] = utc_df.loc[i, "utc_off"] + h
    ...
    df.update(update2)                                              # Update df with them
...
print("Version 70. November 2022.  by Glyn. \n \n")
...
print("Satellites: ", s)
print("User Areas:", n)
print("Handover period:", h, "minute(s)")
...
handovers = ms_df[ms_df["ho_utc"] != "None"]                    # df of handovers
no_handovers = len(handovers)                                   # No of handovers
time_in_handover = no_handovers * h                             # Time in handover

# Outage Analysis

outage_df = ms_df[ms_df["cur_sat"] == "ERR"].copy()             # Get all the outages

outage_df["i2"] = outage_df.index                               # Copy the old index
outage_df = outage_df.reset_index(drop=True)                    # Replaces the old index to make it indexable
no_outages = len(outage_df)

i = 0                                                           # UA counter
outage_total = 0                                                # Set the total outage to zero

p = outage_df.loc[:, "utc_off"]                                 # Get the outage utc_off column
ua_outage = uas.copy()                                          # Create a df of user areas
ua_outage = pd.DataFrame(ua_outage)                             # Turn into a df
ua_outage["outage"] = 0                                         # Add a blank outage column

while i < no_outages:
    outage_region = outage_df.at[i, "ua_name"]                  # get the UA name
    ua_no = outage_df.at[i, "ua_no"]
    outage_start = outage_df.at[i, "utc_off"]                   # Get the outage start time
    outage_index = outage_df.at[i, "i2"]

    outage_index = outage_index + n
    outage_poss_end = ms_df.at[outage_index, "utc_off"]         # Count to the next Epoch

    outage_duration = outage_poss_end - outage_start            # Calculate the outage time
    ua_outage.at[ua_no, "outage"] = ua_outage.at[ua_no, "outage"] + outage_duration        # adds to the total outage

    i += 1

outage_total = ua_outage.loc[:, "outage"].sum(axis=0)
...
print("Outages:")
print("Number of user area outages:", no_outages)
