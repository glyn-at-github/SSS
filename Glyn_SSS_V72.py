...
def allocatesat(s, n, counter, df, sat_names, h):
    ...
    utc_df = df.loc[counter:counter+(n-1), :].copy()            # Create working df for the relevant utc_off epoch
    utc_df = utc_df.reset_index(drop=True)

    # Repeat this loop incrementing i (user area) until all UAs are allocated a satellite
    ...
    while "None" in list(utc_df["cur_sat"]) or "H/O" in list(utc_df["cur_sat"]):   # Repeat until everything allocated
        ...
        while i < n:                                            # Loop through to allocate ERR, H/O and Sat No.
            ...
            if avail_sats_count == 1:
                ...
                # if it's not, then check whether it's an ERR. If not an ERR then it must he a HO & set ho_utc

                if utc_df.at[i, "cur_sat"] != utc_df.loc[i, "prev_sat"]:
                    if utc_df.at[i, "prev_sat"] == "ERR":
                        utc_df.at[i, "ho_utc"] = "None"

                    else:
                        utc_df.at[i, "ho_utc"] = utc_df.loc[i, "utc_off"] + h
                        
                        # Now removes the prev_sat sat from all UAs as well due to handover

                        prev_sat = utc_df.loc[i, "prev_sat"]
                        prev_sat_position = get_position(utc_df.columns, prev_sat)

                        utc_df[prev_sat_position] = 0           # Removes the allocated sat from all UAs
                        utc_df.at[i, sat] = 1                   # Restores the allocated satellite to the chosen UA
...
print("Version 72. November 2022.  by Glyn. \n \n")
...
ua_outage = pd.DataFrame(uas)                                   # get the UA names
ua_outage["outage"] = 0                                         # Set the outage column to zero

while ms_df_counter < ms_df_counter_end:

    if ms_df.loc[ms_df_counter, "cur_sat"] == "ERR":
        index = ms_df_counter % n
        ua_outage.loc[index, "outage"] = ua_outage.loc[index, "outage"] + 1
    ms_df_counter += 1
